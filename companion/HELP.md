...
The SuperJoy does not maintain any internal state about a Group,  Camera, or Preset which might be expected by the user. This can cause
Feedbacks and Variables to have possibly unexpected results. This is best explained by the following scenario.
...
The state of the controller is now Group=1, Camera=2, Preset=2 and Feedbacks and Variables in Companion will reflect that state. The controller did not "remember" that when last in Group 1, the Camera and Preset were 1 and set them back.

***HDMI Output Status***

Users may be confused by the behavior of the HDMI Output Status behavior. The API provides the ability
to set the status behavior by toggling, turning on, or turning off. When executed by Companion as an
Action, this will immediately change the HDMI Output Status and be reflected in any Feedbacks. However, the behavior of selecting a camera in the CGI API is that when a camera is selected, _it also turns the HDMI output on_. So, if you have disabled HDMI Output, then changes cameras, the HDMI Output will be re-enabled. If you want to have it always off, you will need to add additional Actions to each button selecting cameras to also turn off the HDMI output.
